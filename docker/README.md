...
| 文件名 | 用途 |
| :--- | :--- |
| `Dockerfile` | API 服务的镜像构建定义 (基于 Python 3.10-slim)。包含前后端多阶段构建流程。 |
| `docker-compose.yml` | **全栈启动配置**。包含 API 服务及其依赖组件 (Redis) 的编排。 |
| `docker-compose.api.yml` | **独立启动配置**。仅包含 API 服务,需通过环境变量连接外部的数据库和缓存。 |
| `build_linux_x86.sh` | 构建 **x86_64** 服务器镜像 (`linux/amd64`)，需传入版本号，Mac / Linux 均可执行。 |
| `build_linux_arm.sh` | 构建 **ARM64** 服务器镜像 (`linux/arm64`)，需传入版本号。 |
...
API_KEY_ENCRYPTION_SECRET=your-fernet-key-here  # 使用 Fernet.generate_key() 生成
```
...
# 部署到常见 x86 Linux 服务器（Mac 上交叉编译推荐）
...
**若提示 buildx 不可用**（Homebrew docker + Colima 常见）：

```bash
./install-buildx.sh
```

然后重新执行 `./build_linux_x86.sh <version>`。

**若 vite build 报 `Killed` / `cannot allocate memory`**：构建脚本会在 macOS 上自动在宿主机预构建前端；请确保本机已安装 Node.js。
...
构建过程包括：
- 前端构建（Vue 3 + Vite，跨平台时优先宿主机预构建）
- 后端打包（Python 依赖安装，BuildKit pip 缓存）
- 镜像导出（可用于离线部署）
...
| 服务 | 地址 | 说明 |
| :--- | :--- | :--- |
| **管理后台** | http://localhost:8000/ | 前端管理界面（用户管理、API Key 管理、审计日志等） |
| **API 文档** | http://localhost:8000/docs | Swagger UI 交互式 API 文档 |
| **API 接口** | http://localhost:8000/api/v1/* | 数据查询接口（需要 API Key 认证） |
...
```bash
curl -X GET "http://localhost:8000/api/v1/query/real-metrics" \
  -H "X-API-Key: your_api_key_here" \
  -H "Content-Type: application/json"
```

或访问 Swagger 文档进行交互式测试：http://localhost:8000/docs
...
**解决方法**:
1. 确认 `.env` 中配置了 `API_KEY_ENCRYPTION_SECRET`
