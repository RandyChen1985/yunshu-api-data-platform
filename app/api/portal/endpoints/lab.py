...
4. **SQL 规范（必须遵守）**：
   - 每条 SQL 必须在代码最开头用 `--` 注释写明【场景用途】（1-2 行，与 title 呼应）
   - 对关键 JOIN 条件、WHERE 筛选、GROUP BY / 聚合字段补充 `--` 行注释
   - 注释使用中文，便于业务人员阅读；严禁省略注释
   - 示例格式：
     -- 场景：员工薪酬明细查询
     -- 通过 HRMID 关联员工表，筛选 STATUS=1 的有效发薪记录
     SELECT ...
...
输出格式要求：必须仅返回 JSON 列表，格式为 [{{ "title": "场景标题", "description": "场景详细业务描述", "sql": "带注释的 SQL 代码" }}]。不要包含 Markdown 标记或任何前导文字。sql 字段中的换行请使用 \\n 转义。
"""
...
async def suggest_queries(request: SuggestQueriesRequest, user=Depends(require_permission("element:lab:generate"))):
    ...
    messages = [
        {"role": "system", "content": system_prompt},
        {"role": "user", "content": f"请基于当前[{request.mode}]模式上下文，用中文为我推荐 12 个高质量的查询场景。每条 SQL 必须包含 `--` 开头的用途注释和关键逻辑注释，不要输出无注释的裸 SQL。"}
    ]
