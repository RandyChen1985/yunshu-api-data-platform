...
class WeComNotificationService:
    CONFIG_ENABLED = "notify.wecom.enabled"
    CONFIG_WEBHOOK = "notify.wecom.webhook_url"
    CONFIG_SECRET = "notify.wecom.secret"
    CONFIG_ON_REQUEST = "notify.wecom.on_request"
    CONFIG_ON_RESULT = "notify.wecom.on_result"
        ...
        try:
            async with httpx.AsyncClient(timeout=8.0) as client:
                ...
                if isinstance(body, dict) and body.get("errcode") not in (0, None):
                    logger.warning("WeCom API error: %s", body)
                    return False
                return True
    ...
    async def send_test_message(cls, override: Optional[Dict[str, Any]] = None) -> tuple[bool, str]:
        ...
        if override is not None:
            ...
            if not ok:
                return False, "企微返回失败，请检查 Webhook 与加签密钥是否正确"
            return True, ""

        cfg = await cls.get_settings()
        if not cfg.get("enabled"):
            return False, "企微通知未启用，请先保存配置或勾选启用后再测试"
        if not (cfg.get("webhook_url") or "").strip():
            return False, "Webhook 地址为空，请先保存配置"
        ok = await cls.send_markdown(content)
        if not ok:
            return False, "企微返回失败，请检查 Webhook 与加签密钥是否正确"
        return True, ""
