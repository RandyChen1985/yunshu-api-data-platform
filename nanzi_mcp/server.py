...
from nanzi_mcp.client import NanZiApiClient, NanZiApiError, dumps_json
...
async def ensure_mcp_enabled() -> None:
    try:
        ...
        client = NanZiApiClient()
        status = await client.mcp_status()
        if not status.get("enabled"):
            raise RuntimeError(status.get("message") or "MCP Server 未启用")


def _client() -> NanZiApiClient:
    return NanZiApiClient(api_key=get_mcp_api_key() or None)
...
def create_mcp_server(instructions: Optional[str] = None) -> FastMCP:
    ...
    default_instructions = (
        "南孜数据平台：使用 nanzi_list_resources 发现资源，"
        "nanzi_search_metadata 检索元数据，nanzi_query_resource 查询数据。"
    )
    mcp = FastMCP("nanzi-data", instructions=instructions or default_instructions)

    @mcp.tool()
    async def nanzi_list_resources() -> str:
        ...
        except NanZiApiError as exc:
            return f"错误: {exc}"

    @mcp.tool()
    async def nanzi_query_resource(
        resource: str,
        filters_json: str = "[]",
        page: int = 1,
        size: int = 20,
    ...
    @mcp.tool()
    async def nanzi_search_metadata(
        query: str,
